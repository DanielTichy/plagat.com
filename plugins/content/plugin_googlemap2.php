...
//			else
			...
			foreach($fields as $value)
			{
				...
				if($values[0]=='debug'){
					...
					$layer[$this->get_index($values[0], '(')] = $values[1];
				}else if(($brackets=='both'||$brackets=='{')&&preg_match("/layer\[[0-9]+\]/", $values[0])){
					$layer[$this->get_index($values[0], '[')] = $values[1];
			...
			if (!$published )
			{
				...
				if ($lightbox=='1'&&$show==1) {
					$code.="<a href='javascript:void(0)' onclick='javascript:MOOdalBox.open(\"googlemap".$mapnm."\", \"".$lbxcaption."\", \"".$lbxwidth." ".$lbxheight."\", map".$mapnm.", ".((!empty($lbxzoom))?"{zoom : ".$lbxzoom."}":"null").");return false;' class='lightboxlink'>".$txtlightbox."</a>";
				}
				if ($lightbox=='1'&&$show==0) {
					$lbcode.="<a href='javascript:void(0)' onclick='javascript:MOOdalBox.open(\"googlemap".$mapnm."\", \"".$lbxcaption."\", \"".$lbxwidth." ".$lbxheight."\", map".$mapnm.", ".((!empty($lbxzoom))?"{zoom : ".$lbxzoom."}":"null").");return false;' class='lightboxlink'>".$txtlightbox."</a>";
				}
	  ...
	  overmap".$mapnm.".setCenter(map".$mapnm.".getCenter(), c);
				...
				if (count($lookat)>0) {
					// Only works for 1 lookat at the moment
					$la = false;
					$cam = false;
...
//					$opts[] = "zoomlevel : ".$zoom;
			...
			$code = "\n<!-- Plugin Google Maps version 2.12m by Mike Reumer ".(($this->debug_text!='')?$this->debug_text."\n":"")."-->".$code;
