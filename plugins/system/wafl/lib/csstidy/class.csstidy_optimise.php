...
class csstidy_optimise
{
    ...
    function compress_numbers($subvalue)
    {
        ...
        for ($l = 0; $l < count($temp); $l++) {
            ...
            if (!(strlen($temp[$l]) > 0 && (is_numeric($temp[$l] {
                0
            }) || $temp[$l] {
                0
            } == '+' || $temp[$l] {
    ...
    function explode_ws($sep, $string)
    {
        ...
        for ($i = 0, $len = strlen($string); $i < $len; $i++) {
            ...
            case 'str':
                ...
                (isset($output[$num])) ? $output[$num].= $string{$i
            } : $output[$num] = $string{$i
        };
        break;
    }
...
function dissolve_short_bg($str_value)
{
    ...
    for ($i = 0; $i < count($str_value); $i++) {
        ...
        for ($j = 0; $j < count($str_value[$i]); $j++) {
            ...
            } elseif ($str_value[$i][$j] {
                0
            } == '(') {
                $return['background-size'].= substr($str_value[$i][$j], 1, -1) . ',';
            } elseif (in_array($str_value[$i][$j], $pos, TRUE) || is_numeric($str_value[$i][$j] {
                0
            }) || $str_value[$i][$j] {
                0
            } === NULL) {
                $return['background-position'].= $str_value[$i][$j];
                if (!$have['pos']) $return['background-position'].= ' ';
