...
class JMenu extends JObject
{
	...
	function getItems($attribute, $value, $firstonly = false)
	{
		$items = null;
		### Hacked by SDIC: M17n,
		### Menu checks are done here rather than in mainmenu
		global $nowLang, $uid;
		...
		foreach ($this->_items as  $item)
		{
			...
			{
				### Get Params
				$params=$this->getParams($item->id);
				$menuLang=$params->get('lang',false);
				$menuUser=$params->get('users',false);
				$menuUsers=Array();
				if($menuUser!=false)$menuUsers=split('[;|,]',$menuUser);
				### Language Check
				$langCheck=false;
				if($menuLang==$nowLang || $menuLang=='')$langCheck=true;
				### User Check
				$user	= &JFactory::getUser();
				$uid = $user->id;
				$userCheck=false;
				if(count($menuUsers)==0 || in_array($uid, $menuUsers))$userCheck=true;
				if($langCheck && $userCheck){
					if($firstonly) {
						return $item;
					}
					$items[] = $item;
