...
function com_install () {
	...
	$db=JFactory::getDBO();
	$db->setQuery("INSERT INTO `#__modules` ( `id` , `title` , `content` , `ordering` , `position` , `checked_out` , `checked_out_time` , `published` , `module` , `numnews` , `access` , `showtitle` , `params` , `iscore` , `client_id` , `control` ) VALUES ('', 'M17n - Language Selector', '', '7', 'left', '0', '0000-00-00 00:00:00', '1', 'mod_M17n', '0', '0', '0', '', '0', '0', '')");
	$db->query();
	$db->setQuery("INSERT INTO `#__plugins` ( `id` , `name` , `element` , `folder` , `access` , `ordering` , `published` , `iscore` , `client_id` , `checked_out` , `checked_out_time` , `params` ) VALUES ('', 'System - Multilingual (M17n)', 'M17n', 'system', '0', '0', '1', '0', '0', '0', '0000-00-00 00:00:00', '')");
	$db->query();
...
// end hack multilang			
	...
	if($allCopied){
		clearFiles($rootPath);
		$module_msg="Successfully Installed Joomla :: M17n Package";
	}else{
		$db=JFactory::getDBO();
		$db->setQuery("DELETE FROM `#__modules` WHERE title='M17n - Language Selector' AND module='mod_M17n'");
		$db->query();
		$db->setQuery("DELETE FROM `#__plugins` WHERE name='System - Multilingual (M17n)' AND element='M17n' AND folder='system'");
		$db->query();
		
		uncopyFiles($rootPath);
		clearFiles($rootPath);
		
		$module_msg="Unable to Install Joomla :: M17n Package, please read the readme.txt file included with the package before installing.";
	}
...
function copyFiles($basePath){
	...
	echo "<br />Package Files Intalled";
	return true;
}
...
//	if(is_dir($fromPath))$fsdirs->delete($fromPath);
	...
	echo "<br />Installation Files Cleaned";
}
