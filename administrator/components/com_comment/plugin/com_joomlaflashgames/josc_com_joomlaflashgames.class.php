...
class JOSC_com_joomlaflashgames extends JOSC_component {
//	var $_component=''; /* JOSC_component */
//	var	$_id=0; 		/* JOSC_component */
	/* specific properties of REPLACEnewplugin if needed */
//	var _specific_data;

	function JOSC_com_joomlaflashgames($component,&$row,&$list)
	{
    ...
    {	
    	...
    	return  (		$option == 'com_joomlaflashgames' 
    				&& 	$task == 'view'
    			);	
    }
    ...
    function linkToContent($contentId, $commentId='', $joscclean=false, $admin=false)
    {
		...
		$url	= ($admin ? "/" : "" )
				. "index.php?option=com_joomlaflashgames&task=view&id=$contentId" 
    ...
    function cleanComponentCache() {
            mosCache::cleanCache( 'com_joomlaflashgames' );
    }

	/*
	 *  getItemid
	 */
	function getItemid( $component='com_joomlaflashgames') {
    	static $ids;
        if( !isset($ids) ) {
        	$ids = array();
        }
    ...
    {	
		...
		$query 	= "SELECT id, title as title"
				. "\n FROM #__joomlaflashgames_categories"
				. "\n WHERE published = 1"
//				. "\n AND section = 'com_joomlaflashgames' "
//				. "\n AND access >= 0"    
				. "\n ORDER BY ordering"
				;
		$database->setQuery( $query );
	...
	{
		$leftjoin	= "\n LEFT JOIN #__joomlaflashgames  AS $alias ON $alias.id = $contentid ";
		return $leftjoin;
	}
	...
	function mod_commentsGetMostCommentedQuery($secids, $catids, $maxlines)
	{
	    ...
	    $query 	=  	"SELECT COUNT(c.id) AS countid, ct.id, ct.title AS title "
				.	"\n FROM `#__comment`    AS c "
				.  	"\n INNER JOIN `#__joomlaflashgames`  AS ct  ON ct.id = c.contentid "
	       		.  	"\n INNER JOIN `#__joomlaflashgames_categories` AS cat ON cat.id = ct.catid "
				.  	"\n WHERE c.published='1' "
