...
class MenusViewList extends JView
{
	...
	function &_getViewLists()
	{
		...
		// Hack By SDIC: M17n filter
		$filter_lang		= $mainframe->getUserStateFromRequest( "com_menus.$menutype.filter_lang",		'filter_lang',		'',				'string' );
		...
		// Hack By SDIC: M17n filter, build filter list
		// globalize langList
		global $langList;
		// Add Dropdown Default & Language Default
		$choose=Array(
			Array('value'=>'','text'=>'- Select Menu Language -'),
			Array('value'=>'#','text'=>'Universal (not set)')
		);
		$langSel=array_merge($choose,$langList);
		// language filter
		$lists['lang']	= JHTML::_('select.genericlist',  $langSel, 'filter_lang', 'onchange="document.adminForm.submit();"', 'value', 'text', $filter_lang);
