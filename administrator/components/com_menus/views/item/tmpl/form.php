		...
		<tr valign="top">
			...
			<td width="40%">
				<?php
					...
					### >>> SDIC Hack: Menu Childrens Parameters
			//		if($this->item->menutype=='mainmenu'){						
						$paramsData=$this->params;
						$pLang=$paramsData->get('lang','');

						echo $this->pane->startPanel(JText :: _('M17n Parameters'), "context-page");
						echo "
<table width=\"100%\" class=\"paramlist\" cellspacing=\"1\">
<tr><td width=\"40%\" class=\"paramlist_key\"><span class=\"editlinktip\"><label id=\"paramslang-lbl\" for=\"lang\" class=\"hasTip\" title=\"Language::The language \">Language</label></span></td>
	<td class=\"paramlist_value\">";
						
					$defVal = new stdClass(); $defVal->value=""; $defVal->text="- Select Language -";
					global $langList, $nowLang;
					$langList=array_reverse($langList);
					$langList[]=$defVal;
					$langList=array_reverse($langList);
					
					echo JHTMLSelect::genericList( $langList, 'params[lang]', ' id="paramslanguage" class="inputbox"', 'value', 'text', $pLang );
		echo "				
		</td>
</tr>
<tr>
	<td width=\"40%\" class=\"paramlist_key\"><span class=\"editlinktip\"><label id=\"paramskids-lbl\" for=\"kids\" class=\"hasTip\" title=\"Children IDs::A list of children ids seperated by comas and/or dashes (for ranges)\">SubMenu</label></span></td>
	<td class=\"paramlist_value\">
	";
	$kidsList=$this->menutypes;
	$chooseMenu=New stdClass();
	$chooseMenu->menutype='';
	$chooseMenu->title='- Select Children Menu -';
	array_unshift($kidsList,$chooseMenu);
	echo JHTMLSelect::genericList( $kidsList, 'params[kids]', 'class="inputbox" size="1"', 'menutype', 'title', $paramsData->get('kids','') );
	//echo "<input type=\"text\" name=\"params[kids]\" id=\"paramskids\" value=\"".$paramsData->kids."\"  /></td>";
	
	if($paramsData->get('multikey','')=='')$paramsData->set('multikey',$this->item->alias);
	echo "
	</td>
</tr>
<tr>
	<td width=\"40%\" class=\"paramlist_key\"><span class=\"editlinktip\"><label id=\"paramsusers-lbl\" for=\"users\" class=\"hasTip\" title=\"User Access::Specify the user ids separated by comas that can access this item, leave blank if public\">User Access</label></span></td>
	<td class=\"paramlist_value\">
	<input type=\"text\" name=\"params[users]\" id=\"paramsusers\" value=\"".$paramsData->get('users','')."\" /></td>
</tr>
<tr>
	<td width=\"40%\" class=\"paramlist_key\"><span class=\"editlinktip\"><label id=\"paramsmultikey-lbl\" for=\"multikey\" class=\"hasTip\" title=\"Association Key::Specify a common id to use across languages. All menu items which have a corresponding item in another language should use the same key. Alias is used by default\">Association Key</label></span></td>
	<td class=\"paramlist_value\">
	<input type=\"text\" name=\"params[multikey]\" id=\"paramsmultikey\" value=\"".$paramsData->get('multikey','')."\" /></td>
</tr>
</table>";
						echo $this->pane->endPanel();
		//		}
					### End of SDIC Hack: Menu Childrens Parameters	;;;					
