...
class MenusModelList extends JModel
{
	...
	function &getItems()
	{
		...
		// Hack By SDIC: M17n filter, request var retrieval
		...
		// Hack By SDIC: M17n filter, query implementation
		if( isset($filter_lang) && $filter_lang!='' ){
			if($filter_lang!='#'){
				$and .= " AND m.params LIKE '%lang=$filter_lang%'";
			}else{
				// Handle Unset Language
				$and .= " AND (m.params LIKE '%\nlang=\n%' OR m.params NOT LIKE '%\nlang=%')";
			}
		}
		...
		//Language hack:  Load the language files for the various components
